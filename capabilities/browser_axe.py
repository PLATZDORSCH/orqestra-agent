...
Requires the same [browser] extra as other Playwright tools (network access to
load axe from npm CDN unless you extend this to a local bundle).
"""
...
def _handle_axe_wcag_scan(args: dict) -> str:
    ...
    try:
        with playwright_page(url, wait_until=wait_until, timeout_ms=timeout_ms) as (page, response):
            ...
            # Official axe-core browser bundle (same library as npm axe-core)
            page.add_script_tag(url=AXE_MIN_JS_URL)
            ...
            raw_result = page.evaluate(
                ...
                async () => {{
                    if (typeof axe === "undefined" || typeof axe.run !== "function") {{
                        return {{ __error: "axe-core did not load; check network/CDN access" }};
                    }}
    ...
    if isinstance(raw_result, dict) and raw_result.get("__error"):
        return json.dumps(
            {"error": raw_result["__error"], "url": url, "axe_cdn": AXE_MIN_JS_URL},
            ensure_ascii=False,
        )
...
axe_wcag_scan = Capability(
    ...
    description=(
        ...
        "Requires Playwright + network access to load the pinned axe-core script from jsDelivr."
    ),
    parameters={
        "type": "object",
        "properties": {
